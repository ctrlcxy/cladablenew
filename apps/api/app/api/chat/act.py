...
async def execute_act_instruction(
    ...
    try:
        ...
        # Check if project is in failed state
        if project.status == "failed":
            raise HTTPException(status_code=400, detail="Project initialization failed. Please recreate the project.")
        ...
        # Check if project has valid repo path
        if not project.repo_path or not os.path.exists(project.repo_path):
            raise HTTPException(status_code=400, detail="Project repository path is invalid. Please recreate the project.")
