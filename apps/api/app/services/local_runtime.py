...
def start_preview_process(project_id: str, repo_path: Optional[str], port: Optional[int] = None) -> tuple[str, int]:
    """
    ...
    if not repo_path:
        raise RuntimeError(f"Project {project_id} has no valid repository path. Project may have failed to initialize.")
    
    if not os.path.exists(repo_path):
        raise RuntimeError(f"Project repository path does not exist: {repo_path}")
