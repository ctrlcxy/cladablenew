...
def init_git_repo(repo_path: str) -> None:
    """Initialize git repository safely, handling existing git repos"""
    try:
        # Initialize git if not already initialized
        git_init_result = subprocess.run(["git", "init"], cwd=repo_path, capture_output=True, text=True)
        ...
        if status_result.stdout.strip():
            ...
            # If no changes to commit, ensure we have at least one commit
        ...
        ui.debug(f"Git initialization warning: {e}", "Filesystem")
        # Don't fail the entire project creation if git commit fails
        pass
